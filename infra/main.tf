...
# 테라폼 기본 설정
# ===============================================================================
# ec2 생성시 필요한 항목: 키페어 / AMI / 보안그룹 / 서브넷 ID
variable "key_name" {
	description		= "키페어 이름"
	type					= string
	default				= "std19-key"
...
data "aws_security_groups" "security_groups" {
	filter {			# filter 두개를 사용하면 AND
		name = "tag:Name"
		values	= [
			"external-alb-sg",		# ,쓰면 OR
			"internal-ssh-sg"
		]
	}
}

# Node를 추가할 서브넷(IDS) 정보
...
# 적용방법: security_groups_ids	= [data...id, data...id]
# ALB 보안그룹
# data "aws_security_group" "security_group_alb" {
# 	filter {
# 		name	= "tag:Name"
# 		values	= ["external-alb-sg"]
# 	}
# }
...
output "information" {
	value	= {
		vpc_id	= local.vpc_id,
		vpc_security_groups_ids	= data.aws_security_groups.security_groups.ids,
		# security_groups_ids	= [ 
		# 	data.aws_security_group.security_group_alb,
		# 	data.aws_security_group.security_group_ssh
		# ]
...
resource "aws_iam_role_policy" "codepipeline_policy" {
  name	= "${local.tag_header}CodePipelineServicePolicy"
	role	= aws_iam_role.codepipeline_role.id		# 이거덕에 attachment 필요없음
...
resource "aws_launch_template" "asg_lt" {
	...
	image_id				= local.image_id	# data.aws_ami.al2023.id
...
resource "aws_codedeploy_deployment_group" "dg" {
	...
	# 배포 전략(구성) 지정
	# "CodeDeployDefault.AllAtOnce": 타켓 인스턴스 전체에 동시에 한 번에 배포하는 방식
	#																 ( 전체 중단 --> 동시 배포 --> 동시 재시작)
...
resource "aws_codepipeline" "codepipeline" {
	...
	# 소스코드 정보
	artifact_store {
		# 앞서 생성한 Pipeline 전용 S3 버켓 이름 지정
	 ...
	 stage {
		...
		action {
			...
			configuration	= {
				...
				FullRepositoryId	= "hyeon2-dev/ex9-codepipeline"
				# 브랜치 정의
				BranchName	= "main"
			}
		}
	...
	# ===============================================================================
	 stage {
		...
		action {
			...
			version		=	"1"
			# ZIP 소스 압축파일을 다음 스테이지로 전달할 전달용 아티팩트 이름 선언, 된 것을 받겠다
			input_artifacts	= [ "source_output" ]	# State 1의 output_artifacts에 정의된 이름
